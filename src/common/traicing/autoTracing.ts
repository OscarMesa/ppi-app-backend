...
sdk
  .start()
  .then(() =>
    Logger.log('Tracing initialized vendor-coverage-ms', 'autoTracing'),
  )
  .catch((error) =>
    Logger.error(
      'Error initializing tracing vendor-coverage-ms',
      error,
      'autoTracing',
    ),
  );
...
process.on('SIGTERM', () => {
  sdk
    .shutdown()
    .then(() =>
      Logger.log('Tracing terminated vendor-coverage-ms', 'autoTracing'),
    )
    .catch((error) =>
      Logger.log(
        'Error terminating tracing vendor-coverage-ms',
        error,
        'autoTracing',
      ),
    )
